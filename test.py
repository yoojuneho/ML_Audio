...
def extract_mfcc(file_path, n_mfcc=20):
    ...
    mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=n_mfcc)  # 더 많은 MFCC 계수 사용
...
def train_and_evaluate(X, y, test_size=0.2, n_components=64):
    ...
    # GMM 모델 학습
    gmm_male = GaussianMixture(n_components=n_components, covariance_type='full', random_state=42)  # 더 많은 컴포넌트와 full 공분산 사용
